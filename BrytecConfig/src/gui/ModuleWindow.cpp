...
void ModuleWindow::drawModules()
{
    ...
    {
        ImGui::PushID(n);
        drawModule(AppManager::getConfig().getModules()[n]);
...
void ModuleWindow::drawModule(std::shared_ptr<Module>& m)
{
    ...
    ImGui::BeginGroup();
    if(!m->m_enabled) ImGui::PushStyleColor(ImGuiCol_Text, ImGui::GetStyle().Colors[ImGuiCol_TextDisabled]);
    ImGui::PushStyleColor(ImGuiCol_Button, IM_COL32(0, 0, 0, 0));
    ImGui::PushStyleColor(ImGuiCol_ButtonHovered, IM_COL32(0, 0, 0, 0));
    ImGui::PushStyleColor(ImGuiCol_ButtonActive, IM_COL32(0, 0, 0, 0));
    if (ImGui::Button(m->m_name.c_str(), { 150, titleBarHeight })) AppManager::setSelected(m);
    titleHovered = ImGui::IsItemHovered();
    ImGui::PopStyleColor();
    ImGui::PopStyleColor();
    ImGui::PopStyleColor();
    if (!m->m_enabled) ImGui::PopStyleColor();
    ImGui::PushStyleVar(ImGuiStyleVar_FrameRounding, 4.0f);
    ImGui::Indent(5.0f);
    ImGui::SetNextItemWidth(130.0f);
    for (unsigned int i = 0; i < numPins; i++) {
        ImGui::PushID(i);
        if (!m->getPins()[i]->m_enabled) ImGui::PushStyleColor(ImGuiCol_Text, ImGui::GetStyle().Colors[ImGuiCol_TextDisabled]);
        if(ImGui::Button(m->getPins()[i].get()->m_name.c_str(), { 140, 0 })) AppManager::setSelected(m->getPins()[i]);        
        if (!m->getPins()[i]->m_enabled) ImGui::PopStyleColor();
